...
import Complaint, { IComplaintDocument, ComplaintStatus, IComplaintImage } from '../../models/complaint.model';
...
class FieldWorkerService {
  async getAssignedJobs(user: TokenPayload, params: Record<string, any>): Promise<any> {
  ...
  async getJobDetails(user: TokenPayload, id: string): Promise<IComplaintDocument> {
    ...
    if (complaint.assignment?.fieldWorker?.toString() !== user.userId) {
      throw ApiError.forbidden('You are not authorized to view this assignment');
    }
