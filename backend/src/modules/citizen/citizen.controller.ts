...
class CitizenController {
  ...
  async downloadData(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const userId = req.user!.userId;
      const data = await citizenService.downloadData(userId);
      // Return as JSON file
      res.setHeader('Content-Type', 'application/json');
      res.setHeader('Content-Disposition', `attachment; filename="civicpulse_data_${userId}.json"`);
