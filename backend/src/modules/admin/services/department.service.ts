...
class DepartmentService {
  ...
  async updateDepartment(
    admin: TokenPayload,
    deptId: string,
    data: { name: string; description: string; contactInfo: string; status: 'active' | 'inactive' },
    ipAddress?: string,
    userAgent?: string
  ): Promise<IDepartmentDocument> {
  ...
  async deleteDepartment(
    ...
    if (activeComplaints > 0) {
      throw ApiError.badRequest(`Cannot delete department: There are ${activeComplaints} active complaints currently assigned to this department. Please reassign them first.`);
    }
  ...
  async assignOfficer(
    ...
    if (officer.role !== 'officer' && officer.role !== 'field_worker') {
      throw ApiError.badRequest('Only municipal officers or field workers can be assigned to departments');
    }
