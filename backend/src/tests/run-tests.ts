...
const mongoUri = process.env['MONGODB_URI'] || 'mongodb://localhost:27017/civicpulse_test';
...
const runTests = async () => {
  ...
  try {
    ...
    // ───── Test 2: JWT Tokens ─────
    try {
      const payload = { userId: '507f1f77bcf86cd799439011', email: 'officer@test.com', role: 'officer' };
      const tokens = generateTokenPair(payload);
      const verified = verifyAccessToken(tokens.accessToken);
      
      const passed = verified.email === payload.email && verified.role === payload.role;
    ...
    try {
      ...
      logTest('AI Keyword Classification & Priority Routing', passed, `Category: ${category}, Priority: ${priority}`);
    } catch (e: any) {
    ...
    try {
      ...
      const passed = fetched !== null && fetched.departments[0] === 'Sanitation' && fetchedAfterDelete === null;
      logTest('In-Memory SimpleCache Performance & Invalidation', passed);
    } catch (e: any) {
      logTest('In-Memory SimpleCache Performance & Invalidation', false, e.message);
    }
    ...
    try {
      ...
      const testComplaint = await Complaint.create({
        ...
        timeline: [{ status: 'submitted', title: 'Submitted', description: 'Report created', timestamp: new Date() }]
      });

      const userExists = !!(await User.exists({ email: 'citizen@test.com' }));
      const complaintExists = !!(await Complaint.exists({ title: '[TEST] Broken water main leak' }));
