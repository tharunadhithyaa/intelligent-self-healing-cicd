...
@Component({
  ...
  template: `
    <div class="notifications-page animate-fade-in-up">
      <app-page-header
        ...
        icon="notifications"
      >
        <button mat-stroked-button color="primary" (click)="notificationsService.markAllAsRead()" [disabled]="notificationsService.unreadCount() === 0">
        ...
        @for (notif of notificationsService.notifications(); track notif._id) {
          <div class="notification-item" [class.unread]="!notif.isRead">
            ...
            <div class="notification-actions">
              @if (!notif.isRead) {
                <button mat-icon-button color="primary" (click)="notificationsService.markAsRead(notif._id)" title="Mark as Read">
                  <mat-icon>check_circle</mat-icon>
                </button>
              }
              <button mat-icon-button color="warn" (click)="notificationsService.deleteNotification(notif._id)" title="Delete Notification">
  ...
  styles: [`
    ...
    .notification-icon {
      ...
      &.system_alert { color: $danger; background: rgba(220, 38, 38, 0.1); }
      &.status_update { color: $primary; background: rgba(22, 163, 74, 0.1); }
      &.announcement { color: #2563eb; background: rgba(37, 99, 235, 0.1); }
    }
