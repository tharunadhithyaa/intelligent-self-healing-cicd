...
pipeline {
    ...
    environment {
        ...
        // Single pipeline image tag (set to BUILD_NUMBER in Stage 1)
        IMAGE_TAG           = ''
        ...
        stage('Trivy Image Scan') {
            steps {
                ...
                script {
                    ...
                    def imagesToScan = [
                        "${env.DOCKER_IMAGE_PREFIX}/backend:${env.IMAGE_TAG}",
                        "${env.DOCKER_IMAGE_PREFIX}/frontend:${env.IMAGE_TAG}",
                        "${env.DOCKER_IMAGE_PREFIX}/nginx:${env.IMAGE_TAG}",
                        "${env.DOCKER_IMAGE_PREFIX}/mongodb:${env.IMAGE_TAG}"
                    ]
