...
2. **Technical Explanation:** Git push triggers Jenkins Poll SCM. Jenkins executes 21 stages: checkout, environment validation, parallel dependency install, static linting, application build, unit tests with coverage, SonarQube analysis, SonarQube quality gate, Trivy filesystem security scanning, parallel Docker BuildKit compilation, Trivy container image security gates, multi-attempt image push to GHCR, zero-commit Argo CD parameter override patching (`update-gitops.sh`), pre-deployment image manifest verification, pre-deployment cluster health gate, post-deployment health verification (`health-check.sh`), monitoring stack verification (`verify-monitoring.sh`), Prometheus scrape targets audit, ML Decision Controller verification, deployment report generation, and monitoring report archiving. Prometheus scrapes `/metrics` and Kubernetes telemetry; Alertmanager dispatches firing alerts via HTTP POST to `/api/v1/alerts` on the `civicpulse-ml-decision-controller` microservice; the controller computes severity scores, checks `PersistentCooldownStore` (ConfigMap/Redis/Mongo), invokes `KubernetesActionHandler` to execute API mutations, and polls `ClosedLoopVerifier` for pod readiness and HTTP status 200 health responses.
